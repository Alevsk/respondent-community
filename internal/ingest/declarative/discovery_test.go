...
func TestDiscoveryRejectsInvalidSourceConfigurations(t *testing.T) {
	for _, discovery := range []string{
		...
		if _, err := loadMediaTestSource(t, yaml); err == nil { t.Errorf("accepted %s",discovery) }
	}
}

type fakeSRVResolver struct { records []*net.SRV; calls int; err error }
...
func TestDiscoveryCachesAndValidatesTargets(t *testing.T) {
	...
	for i:=0;i<2;i++ {
		...
		if err!=nil || len(origins)!=1 || origins[0]!="https://good.api.example.com" { t.Fatalf("origins=%v err=%v",origins,err) }
	}
	if dns.calls!=1 { t.Fatalf("DNS calls=%d",dns.calls) }
	now=now.Add(2*time.Minute)
	dns.err=fmt.Errorf("DNS unavailable")
	if _,err:=r.Origins(context.Background());err==nil { t.Fatal("expired discovery survived DNS failure") }
	ctx,cancel:=context.WithCancel(context.Background());cancel()
	if _,err:=r.Origins(ctx);err==nil { t.Fatal("canceled discovery succeeded") }
}
...
func TestDiscoveryTransportRotatesHostsPreservingPathAndQuery(t *testing.T) {
	...
	client:=&http.Client{Transport:mediaRoundTripper(func(r *http.Request)(*http.Response,error){
		...
		if r.URL.RequestURI()!="/catalog?offset=0&limit=2" { t.Fatalf("path/query changed: %s",r.URL) }
		if r.URL.Host=="first.api.example.com" { return nil,fmt.Errorf("offline") }
	...
	if err!=nil || string(b)!="[]" || len(hosts)!=2 { t.Fatalf("hosts=%v body=%s err=%v",hosts,b,err) }
}
...
func TestDiscoveredPaginationDoesNotPublishPartialOrMixMirrors(t *testing.T) {
	...
	client:=&http.Client{Transport:mediaRoundTripper(func(r *http.Request)(*http.Response,error){
		...
		if r.URL.Host=="first.api.example.com" && r.URL.Query().Get("offset")=="1" { return nil,fmt.Errorf("page failed") }
		body:=`[]`
		if r.URL.Query().Get("offset")=="0" { body=`[{"id":"`+r.URL.Host+`","lat":1.0,"lon":2.0,"alt":0.0,"time":1,"speed":0.0}]` }
  ...
  pagination:
	...
	cs,err:=loadMediaTestSource(t,yaml);if err!=nil{t.Fatal(err)}
	a,err:=NewDeclarativeAdapterWithClient(cs,logging.NewNopLogger(),client);if err!=nil{t.Fatal(err)}
	...
	if err:=a.fetchAndProcess(context.Background());err!=nil{t.Fatal(err)}
	entities,_,err:=a.Snapshot(context.Background());if err!=nil{t.Fatal(err)}
	if len(entities)!=1 || entities[0].ExternalID!="second.api.example.com" {t.Fatalf("mixed/partial entities: %+v",entities)}
	if strings.Join(requests,",")!="first.api.example.com:0,first.api.example.com:1,second.api.example.com:0,second.api.example.com:1" {t.Fatalf("requests=%v",requests)}
}
